...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .table-wrap {{ max-height: 430px; overflow: auto; border: 1px solid var(--line); border-radius: 7px; }}
...
<body>
  ...
  <main class="wrap">
    <section class="filters" aria-label="Filtros">
      ...
      <div><label for="projeto">Projeto</label><select id="projeto"></select></div>
    ...
    <section class="panel" style="margin-top:14px">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>OS</th><th>Projeto</th><th>Defeito</th><th>SE</th><th>Instalação</th><th>Trecho</th><th>Taxonomia</th><th>Execução</th><th>Registro</th></tr></thead>
  ...
  <script>
    ...
    const state = {{
      ...
      tipoTrecho: "", taxonomia: "", projeto: "", crit: "", month: ""
    }};
    ...
    function resetControls() {{
      ...
      Object.assign(state, {{dateStart: dates[0] || "", dateEnd: dates[dates.length - 1] || "", instalacao:"", se:"", execucao:"", tipoTrecho:"", taxonomia:"", projeto:"", crit:"", month:""}});
      ...
      initSelect("projeto", r => r.projeto, "Todos");
      updateDataStatus();
    }}
    ...
    function filtered() {{
      return DATA.filter(r => {{
        ...
        if (state.projeto && norm(r.projeto) !== state.projeto) return false;
    ...
    function renderActiveFilters() {{
      const chips = [];
      const labels = {{instalacao:"Instalação", se:"SE", execucao:"Execução", tipoTrecho:"Trecho", taxonomia:"Taxonomia", projeto:"Projeto"}};
    ...
    function renderTable(rows) {{
      const ordered = [...rows].sort((a, b) => String(a.dataRegistro || "").localeCompare(String(b.dataRegistro || "")));
      els("detailRows").innerHTML = ordered.map(r => `<tr><td>${{escapeHtml(r.id)}}</td><td>${{escapeHtml(r.os)}}</td><td>${{escapeHtml(r.projeto)}}</td><td>${{escapeHtml(short(r.defeito, 52))}}</td><td>${{escapeHtml(r.se)}}</td><td>${{escapeHtml(r.instalacao)}}</td><td>${{escapeHtml(r.tipoTrecho)}}</td><td>${{escapeHtml(r.taxonomia)}}</td><td>${{escapeHtml(statusOf(r))}}</td><td>${{escapeHtml(r.dataRegistro)}}</td></tr>`).join("");
    }}
