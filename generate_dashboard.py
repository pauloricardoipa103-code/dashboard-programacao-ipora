...
<body>
  ...
  <script>
    ...
    function renderTable(rows) {{
      ...
      els("detailRows").innerHTML = ordered.map(r => `<tr><td>${{escapeHtml(r.id)}}</td><td>${{escapeHtml(r.os)}}</td><td>${{escapeHtml(short(r.defeito, 52))}}</td><td>${{escapeHtml(r.se)}}</td><td>${{escapeHtml(r.instalacao)}}</td><td>${{escapeHtml(r.tipoTrecho)}}</td><td>${{escapeHtml(r.taxonomia)}}</td><td>${{escapeHtml(statusOf(r))}}</td><td>${{escapeHtml(r.dataRegistro)}}</td></tr>`).join("");
    }}
